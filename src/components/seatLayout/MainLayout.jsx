...
const MainLayout = ({
  ...
  const handleSeatSelection = (seat, cart) => {
    setSelectedSeats((prevSeats) => {
      ...
      if (isAlreadySelected) {
        ...
        return [
          ...prevSeats,
          { cart, number: seat.number, class: currentClass },
        ];
      }
    });
  };
  ...
  const totalPrice = selectedSeats.reduce(
    (total, seat) => total + classPrices[seat.class],
    0
  );
  ...
  const handleOpenCheckout = async () => {
    ...
    try {
      const response = await axios.post(
        `${BASE_URL}/booking/user/create/booking`,
        bookingData,
        {
          headers: {
  ...
  const handleRemoveSeat = (seatToRemove) => {
    setSelectedSeats((prevSeats) =>
      prevSeats.filter(
        (seat) =>
          seat.cart !== seatToRemove.cart ||
          seat.number !== seatToRemove.number ||
          seat.class !== seatToRemove.class
      )
    );
  };
      ...
      <div className="flex flex-col w-1/5 p-4 bg-gray-100 rounded-lg shadow-lg">
        ...
        <ul className="list-none">
          {selectedSeats.map((seat, index) => (
            ...
            >
              <span>
                Class {seat.class} Seat {seat.number}{" "}
                <p className="text-red-600">Rs {classPrices[seat.class]}</p>
              </span>
              <button
        ...
        <h2 className="text-red-600 mt-5 text-xl font-semibold ">
          {" "}
          Price : Rs.{totalPrice}
        </h2>
        <button
