...
const OrderHistory = () => {
    ...
    useEffect(() => {
        const fetchTickets = async () => {
            try {
                const response = await axios.get(`${baseURL}/booking/user/search/tickets`, {
                    headers: {
    ...
    return (
        <div className="p-4">
            <h1 className="text-2xl font-semibold mb-4 text-blue-600">Order History</h1>
                ...
                <tbody>
                    {tickets.map(ticket => (
                        <React.Fragment key={ticket.ticketNumber}>
                            ...
                            >
                                <td className="text-center py-2 px-4 border-b">{ticket.ticketNumber}</td>

                                <td className="py-2 text-center px-4 border-b">{ticket.origin}</td>
                                <td className="py-2 text-center px-4 border-b">{ticket.destination}</td>
                                <td className="py-2 text-center px-4 border-b">{ticket.departureTime}</td>
                                <td className="py-2 text-center px-4 border-b">{ticket.status}</td>
                            </tr>
                            {expandedRows.includes(ticket.ticketNumber) && (
                                <tr>
                                    <td colSpan="6" className="py-2 px-4 border-b bg-gray-50">
                                        <div>
                                            <p><strong>Booking Reference ID:</strong> {ticket.bookingRefID}</p>
                                            <p><strong>Class:</strong> {ticket.class}</p>
                                            <p><strong>Booked User:</strong> {ticket.bookedUser}</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
